...
def fetch_dataset(pickle_path, lyrics_path, force=False, as_dict=False, clean_genre=True, clean_lyrics=True) -> typing.Union[pd.DataFrame, typing.Dict[str, np.ndarray]]:
    if pickle_path is not None and os.path.exists(pickle_path) and not force:
        ...
        dataset = add_chart_info(dataset, 'charts.csv')
